...
class App extends React.Component {
  ...
  getWeather = async () => {
    ...
    // const WEATHER = `https://city-explorer-api9.herokuapp.com/getweather?searchQuery=Amman`;
      ...
      <>
        ...
        {this.state.showInfo && (
          <>
            ...
            <Image
              src={`https://maps.locationiq.com/v3/staticmap?key=f5de8e48adbdc6&center=${this.state.result.lat},${this.state.result.lon}&zoom=10`}
              alt="city"
              width={800}
              height={825}
              id="map"
            ...
            {this.state.forecastArr.map((info) => {
              return <Weather forecastArr={info} />;
            })}
            {this.state.forecastArrMovies.map((info) => {
              return <Movies forecastArrMovies={info} />;
            })}
          </>
        )}
